...
describe("metadata pipeline invariants", () => {
  ...
  it("treats omitted legacy readiness flags as already satisfied during resync checks", () => {
    const shouldResyncAsset = (input: {
      isNew: boolean;
  ...
  it("normalizes tall person-sized detections down to a head crop", () => {
    ...
    expect(normalizeTallBox({ x: 0.18, y: 0.1, w: 0.2, h: 0.62 })).toEqual({
      x: 0.11880000000000002,
      y: 0.12579200000000001,
      w: 0.3224,
      h: 0.3224,
    });
  });
